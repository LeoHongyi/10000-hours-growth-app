  ...
  return (
    <section className="stack-md">
      <section className="page-card stack-xs">
        <p className="eyebrow">本月计划概览</p>
        <h1>计划</h1>
        <p>本月已投入 {formatDuration(monthlyMinutes)}</p>
      </section>

      <section className="page-card stack-md">
        <h2>今日建议</h2>
        {todayPlans.length === 0 ? (
          <p className="muted">今天的建议已经处理完了，去首页开始下一段专注。</p>
        ) : (
          todayPlans.map((plan) => (
      ...
      <section className="page-card stack-md">
        ...
        {weeklyPlans.length === 0 ? (
          <p className="muted">下周框架会在生成后显示在这里。</p>
        ) : (
          weeklyPlans.map((plan) => (
            <article key={plan.id} className="list-row">
              ...
              <span>{plan.suggestedMinutes} 分钟</span>
            </article>
          ))
        )}
      </section>
